...
import {StyleTranfer} from './components/pages/style_transfer';
import {base64ImageToTensor, tensorToImageUrl, resizeImage, toDataUri} from './image_utils';
import * as tf from '@tensorflow/tfjs';


export default class WebcamDemo extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      mode: 'results',
  ...
  async handleCameraCapture() {
    ...
    image = await resizeImage(image.uri, 240).catch(err => console.log('err'));
    console.log('=================')
    ...
    if(mode === 'newStyleImage' && image.base64 != null) {
      ...
      if(contentImage == null) {
        ...
        resultImage = await this.stylize(contentImage, styleImage).catch(err => console.log(err)),
        this.setState({
          styleImage,
          contentImage,
          resultImage,
